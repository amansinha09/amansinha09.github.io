...
Formally, let X belongs to <img src="https://render.githubusercontent.com/render/math?math=R^{n x m}"> and Y belongs <img src="https://render.githubusercontent.com/render/math?math=R^{n x p}"> be two zero mean matrices of n observations with m and p features respectively. We aim to find K orthogonal linear projections <img src="https://render.githubusercontent.com/render/math?math=A = [a_1, a_2, .., a_k], B = [b_1, b_2, .., b_k]">
such that:

<img src="https://render.githubusercontent.com/render/math?math=(a^{*},b^{*}) = argmax(corr(a^{T}X,b^{T}Y)) = argmax \frac{a^{T}\sum_{XX}a}{\sqrt{a^{T}\sum_{XY}a}\sqrt{b^{T}\sum_{YY}b}}">
