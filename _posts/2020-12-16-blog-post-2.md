...
Formally, let X belongs to $R^{n x m}$ and Y belongs $R^{n x p}$ be two zero mean matrices of n observations with m and p features respectively. We aim to find K orthogonal linear projections A = [$a_1, a_2, .., a_k$], B = [$b_1, b_2, .., b_k$]
such that:

($a^{*},b^{*}$) = argmax corr($a^{T}X,b^{T}Y$) = argmax $\frac{a^{T}\sum_{XX}a}{\sqrt{a^{T}\sum_{XY}a}\sqrt{b^{T}\sum_{YY}b}}$

where $\sum_{XX}$ and $\sum_{YY}$ are covariance matrices of X and Y respectively and $\sum_{XY}$ is their cross covarience matrix.
...
[1] Lisa Bonheme and Marek Grzes; SESAM at SemEval-2020 Task 8: Investigating the relationship betweenimage and text in sentiment analysis of memes; Proceedings of the 14th International Workshop on Semantic Evaluation, pages 804–816Barcelona, Spain (Online), December 12, 2020.
